"use client";

import { useState, useEffect } from "react";

export default function ThemeSwitcher() {
  const [theme, setTheme] = useState("light");

  useEffect(() => {
    // Retrieve theme from localStorage or fallback to system preference
    const storedTheme = localStorage.getItem("theme");
    if (storedTheme) {
      setTheme(storedTheme);
      document.documentElement.classList.toggle("dark", storedTheme === "dark");
    } else {
      const isDark = window.matchMedia("(prefers-color-scheme: dark)").matches;
      setTheme(isDark ? "dark" : "light");
      document.documentElement.classList.toggle("dark", isDark);
    }
  }, []);

  const toggleTheme = () => {
    const newTheme = theme === "light" ? "dark" : "light";
    setTheme(newTheme);
    document.documentElement.classList.toggle("dark", newTheme === "dark");
    localStorage.setItem("theme", newTheme); // Store theme in localStorage
  };

  return (
    <div
      onClick={toggleTheme}
      className={`relative flex items-center justify-between w-16 h-8 px-1 rounded-full cursor-pointer transition-all ${
        theme === "light"
          ? "bg-gray-800 text-gray-400"
          : "bg-gray-300 text-gray-900"
      }`}
    >
      {/* Moon Icon */}
      <svg
        viewBox="0 0 384 512"
        xmlns="http://www.w3.org/2000/svg"
        className={`w-4 h-4 absolute left-2 transition-opacity duration-300 ${
          theme === "light" ? "opacity-100" : "opacity-100"
        }`}
      >
        <path
          d="M223.5 32C100 32 0 132.3 0 256S100 480 223.5 480c60.6 0 115.5-24.2 155.8-63.4c5-4.9 6.3-12.5 3.1-18.7s-10.1-9.7-17-8.5c-9.8 1.7-19.8 2.6-30.1 2.6c-96.9 0-175.5-78.8-175.5-176c0-65.8 36-123.1 89.3-153.3c6.1-3.5 9.2-10.5 7.7-17.3s-7.3-11.9-14.3-12.5c-6.3-.5-12.6-.8-19-.8z"
          fill="currentColor"
        />
      </svg>

      {/* Sun Icon */}
      <div
        className={`absolute right-2 w-4 h-4 rounded-full bg-yellow-400 transition-opacity duration-300 ${
          theme === "dark" ? "opacity-100" : "opacity-100"
        }`}
      ></div>

      {/* Toggle Button */}
      <div
        className={`absolute w-6 h-6 bg-white rounded-full shadow-md transition-all ${
          theme === "light" ? "left-1" : "left-9"
        }`}
      ></div>
    </div>
  );
}
