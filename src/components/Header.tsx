...
export default function Header() {
  ...
  const headerVariants = {
    hidden: { x: "-50%", y: "-130%", transition: { duration: 0.3 } },
    visible: { x: "-50%", y: "0%", transition: { duration: 0.3 } },
  };
  ...
  <>
    {navlinks.map((link, index) => (
      <Link
        key={index}
        href={link.href}
        className="hover:bg-gradient-to-r dark:text-white text-black hover:from-blue-500 hover:to-purple-500 hover:bg-clip-text hover:!text-transparent transition-colors duration-300"
      >
        {link.name}
      </Link>
    ))}
